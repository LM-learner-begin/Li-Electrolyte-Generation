...
def optimization(model, ld_kwargs, data_loader,
                 num_starting_points=100, num_gradient_steps=300,
                 lr=1e-3, num_saved_crys=10):
    # num_gradient_steps=5000 is default,300 is usually sufficient for the property to converge
    ...
    for i in tqdm(range(num_gradient_steps)):
        opt.zero_grad()
    # ------------------------------------
        # multi-property optimization loss
        preds = model.fc_property(z) # output shape (num_starting_points, num_properties) 
        # compute the L2 regularization penalty on z to prevent it from running too far away from the origin, which can cause the generated crystals to be unrealistic.
        # compute the square and mean of each z vector, the farther z goes from the origin, the larger the penalty
        alpha = 0.1  # regularization strength. recommended to keep at 0.1
        z_penalty = alpha * torch.mean(torch.sum(z**2, dim=1))


        if preds.shape[1] == 2:
            energy_pred = preds[:, 0] # formation energy prediction
            bandgap_pred = preds[:, 1] # band gap prediction
            bg_weight = model.hparams.get("bandgap_weight",1.0)
            
            prop_loss = (energy_pred - bg_weight * bandgap_pred).mean()
            # Add the L2 regularization penalty to the loss
            loss = prop_loss + z_penalty
        else: # adjustable weight for band gap in the loss
