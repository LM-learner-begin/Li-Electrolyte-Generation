...
class CDVAE(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # multi-property prediction can be easily achieved by changing the output dimension of this MLP and the target in dataset.

        if self.hparams.predict_property:
            # 获取输出维度
            try:
            # 直接去读取传入的数据配置中的 num_targets (无论是 1, 2 还是 6)
                out_dim = self.hparams.data.num_targets
            except AttributeError:
            # 如果配置里压根没有 num_targets，我们宁愿让它明确报错，也不让它瞎猜！
                raise ValueError(
                "模型不知道该输出几个属性！请确保你的配置文件 (如 li_prop.yaml) 中包含 'num_targets' 参数！"
            )

            self.fc_property = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                         self.hparams.fc_num_layers, out_dim)
    ...
    def property_loss(self, z, batch):
        # 修改代码适应带隙和形成能两个属性
        preds = self.fc_property(z)  # shape (N, num_targets)，num_targets=2 for 双属性预测
        targets = batch.y  # shape (N, num_targets)
        
        # judge if it's single property or multi-property prediction based on the shape of targets
        if preds.shape[1] == 2:
            # preds[:, 0] is formation energy, preds[:, 1] is band gap
            loss_energy = F.mse_loss(preds[:, 0], targets[:, 0])
            loss_bandgap = F.mse_loss(preds[:, 1], targets[:, 1])
            bg_weight = self.hparams.get("bandgap_weight", 1.0) # 可以调整这个权重来平衡两个属性的损失
            
            # ========= 修改：挂载到进度条  =========
            # 判断当前是训练 (train) 还是验证 (val) 阶段，动态加前缀
            mode = "train" if self.training else "val"
            
            # 使用 self.log 将两个分离的 loss 强制显示在进度条上 (prog_bar=True)
            self.log(f"{mode}_energy_loss", loss_energy, prog_bar=True, on_step=True)
            self.log(f"{mode}_bg_loss", loss_bandgap, prog_bar=True, on_step=True)
            # ========= 修改结束 =========

            # 原样返回加权求和的结果，衔接外部的总 loss 相加逻辑
            return loss_energy + bg_weight * loss_bandgap
        else:
            loss_prop = F.mse_loss(preds, targets)
            
            mode = "train" if self.training else "val"
            self.log(f"{mode}_prop_loss", loss_prop, prog_bar=True, on_step=True, batch_size=batch.num_graphs)
